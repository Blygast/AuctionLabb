        ...
        <section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {images.map((att) => (
              ...
              >
                <img src={`${SERVER_URL}${att.url}`} alt={att.fileName} className="w-full h-32 object-cover" />
            ...
            {videos.map((att) => (
              <div key={att.id} className="rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700">
                <video controls className="w-full max-h-64" preload="metadata">
                  <source src={`${SERVER_URL}${att.url}`} type={att.contentType} />
                </video>
                <div className="p-2 bg-gray-50 dark:bg-gray-700 flex items-center justify-between">
                  ...
                  <a href={`${SERVER_URL}${att.url}`} download className="text-xs text-primary-600 hover:underline dark:text-primary-500">
                    Download
                  </a>
                </div>
              </div>
          ...
          <ul className="space-y-2">
            {documents.map((att) => (
              <li key={att.id}>
                ...
                >
                  <span className="text-gray-400 dark:text-gray-500 mr-3">{fileIcon(att.contentType)}</span>
