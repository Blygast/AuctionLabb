...
import { ROUTES, isPathActive } from '../routes';
...
export default function Sidebar() {
  ...
  const linkClass = (path: string) =>
    `flex items-center p-2 text-base font-normal text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 ${
      isPathActive(location.pathname, path) ? 'bg-gray-100 dark:bg-gray-700' : ''
    }`;
  ...
  return (
    <>
          ...
          <div className="flex flex-col flex-1 pt-5 pb-4 overflow-y-auto">
            <div className="flex-1 px-3 bg-white dark:bg-gray-800">
              ...
              <ul className="space-y-1">
                <li><Link to={ROUTES.auctions} className={linkClass(ROUTES.auctions)}>
                ...
                {isAuthenticated && (
                  <li><Link to={ROUTES.create} className={linkClass(ROUTES.create)}>
                    <svg className="w-6 h-6 text-gray-500 transition duration-75 dark:text-gray-400" fill="currentColor" viewBox="0 0 20 20">
              ...
              <ul className="space-y-1">
                {!isAuthenticated ? (
                  <>
                    <li><Link to={ROUTES.login} className={linkClass(ROUTES.login)}>
                      <svg className="w-6 h-6 text-gray-500 transition duration-75 dark:text-gray-400" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clipRule="evenodd" />
                      </svg>
                      <span className="ml-3">Sign In</span>
                    </Link></li>
                    <li><Link to={ROUTES.register} className={linkClass(ROUTES.register)}>
                      <svg className="w-6 h-6 text-gray-500 transition duration-75 dark:text-gray-400" fill="currentColor" viewBox="0 0 20 20">
                  ...
                  <li><Link to={ROUTES.profile} className={linkClass(ROUTES.profile)}>
                    <svg className="w-6 h-6 text-gray-500 transition duration-75 dark:text-gray-400" fill="currentColor" viewBox="0 0 20 20">
              ...
              {isAdmin && (
                <>
                  {sectionHeader('Admin')}
                  <ul className="space-y-1">
                    <li><Link to={ROUTES.admin} className={linkClass(ROUTES.admin)}>
                      <svg className="w-6 h-6 text-gray-500 transition duration-75 dark:text-gray-400" fill="currentColor" viewBox="0 0 20 20">
      ...
      <div className="fixed inset-0 z-10 hidden bg-gray-900/50 dark:bg-gray-900/90" id="sidebarBackdrop" onClick={closeSidebar} />
    </>
  );
}
